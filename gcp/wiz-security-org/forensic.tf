resource "google_organization_iam_custom_role" "wiz_security_role_forensic_ext" {
...
resource "google_organization_iam_member" "wiz_worker_security_role_forensic_ext" {
  count      = var.forensic ? 1 : 0
  org_id     = var.org_id
  role       = google_organization_iam_custom_role.wiz_security_role_forensic_ext[0].name
  member     = "serviceAccount:${local.disk_analysis_service_account_id}"
  depends_on = [google_organization_iam_custom_role.wiz_security_role_forensic_ext]
  condition {
