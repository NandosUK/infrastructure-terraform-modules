...
module "lb-http" {
  ...
  managed_ssl_certificate_domains = [ var.domain_name != null ? "${var.domain_name}.${var.domain_host}" : var.environment == "prod" ? "${var.name}.${var.domain_host}" : var.environment == "preview" ? "${var.name}-preview.${var.domain_host}" : "${var.name}-preprod.${var.domain_host}"]
